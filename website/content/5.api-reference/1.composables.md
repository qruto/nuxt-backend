...
// Billing. `gift` opens a checkout billed to someone else's email.
function useBilling(): { subscription, isSubscribed, checkout, gift, portal, changePlan, cancel, ... }
function useFeatures(): { has, hasPlan }
function useCredits(): { balance, topUp, gift, refresh }
